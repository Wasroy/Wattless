...
export const api = {
  ...
  priceCurve: async (regionId: string): Promise<PriceCurvePoint[]> => {
    const raw = await request<{
      data: Array<{ hour: string; spot: number; ondemand: number; source: string }>;
    }>(`/api/prices/curve?region_id=${regionId}`);
    const now = new Date().getHours();
    return raw.data.map((p) => ({
      label: p.hour,
      spot_price: p.spot,
      ondemand_price: p.ondemand,
      is_current: parseInt(p.hour) === now,
    }));
  },
